...
export class HistoryTarget implements ValueObject<HistoryTarget> {
    constructor(private readonly _postId: string, private readonly _kind: string) {
        if (!regExpUUID.test(this.postId) || !this.kind) throw new Error('Invalid history target')
    }
