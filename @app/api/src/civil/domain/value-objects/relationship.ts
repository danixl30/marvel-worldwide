...
export class CivilRelationship implements ValueObject<CivilRelationship> {
    constructor(private readonly _targetId: string, private readonly _kind: string) {
        if (!regExpUUID.test(this.targetId) || !this.kind || this.kind.length < 5) throw new Error('invalid relationship')
    }
