...
import { getVillainDetailsLogic } from '../logic/villain.details'
import { getUserState } from '../../global-state/user/UserContext'

export default function VillainDetailsPage() {
    const userState = getUserState()
    const { data, error, isLoading, onClickHeroe, onClickDetials } =
        getVillainDetailsLogic(
            nativeOnInitJob(
                useRefStateFactory(),
    ...
    return (
        <>
            <Container>
                ...
                <Spacer />
                <Text h4>Enemies:</Text>
                {data.value &&
                    data.value.enemies.map((ene) => (
                        <Link block onClick={() => onClickHeroe(ene.id)}>
                            {ene.name}
                        </Link>
                    ))}
                <Spacer y={5} />
